...
st.title("🍎 Diet Recommendations by Disease")

def app():
  # Disease → Diet dictionary
 disease_diet = {'Fungal infection': ['Antifungal Diet', 'Probiotics', 'Garlic', 'Coconut oil', 'Turmeric'],
 ...
 # Select disease
 selected_disease = st.selectbox("Select a disease:", list(disease_diet.keys()))
 
 # Show diet recommendations
 if selected_disease:
     st.subheader(f"Recommended Diet for {selected_disease}:")
     for item in disease_diet[selected_disease]:
         st.write(f"✅ {item}")
