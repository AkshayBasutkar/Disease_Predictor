...
st.title("🍎 Medicine Recommendations by Disease")

def app():

    medicines = {'Fungal infection': ['Antifungal Cream', 'Fluconazole', 'Terbinafine', 'Clotrimazole', 'Ketoconazole'],
    ...
    # Select disease
    selected_disease = st.selectbox("Select a disease:", list(medicines.keys()))
    
    # Show diet recommendations
    if selected_disease:
        st.subheader(f"Recommended Medicine for {selected_disease}:")
        for item in medicines[selected_disease]:
            st.write(f"✅ {item}")
